...
class PipelineRunner():
    ...
    def information_gathering_preprocess(self):
        ...
        self.memory.update_working_area(input)
    ...
    def information_gathering_postprocess(self, response):
        ...
        current_augmentation = self.memory.get_recent_history(constants.AUGMENTED_IMAGES_MEM_BUCKET, k=1)[0]
        subtask_description = self.memory.get_recent_history(constants.SUBTASK_DESCRIPTION, k=1)[0]
    ...
    def task_inference_preprocess(self):
        ...
        input[constants.PREVIOUS_ACTION] = self.memory.get_recent_history(constants.ACTION, k=1)[0]
        input[constants.EXECUTING_ACTION_ERROR] = self.memory.get_recent_history(constants.ACTION_ERROR, k=1)[0]
    ...
    def action_planning_preprocess(self):
        ...
        if pre_action:
            input[constants.PREVIOUS_ACTION] = self.memory.get_recent_history(constants.ACTION, k=1)[0]
            input[constants.PREVIOUS_REASONING] = self.memory.get_recent_history(constants.DECISION_MAKING_REASONING, k=1)[0]
            input[constants.KEY_REASON_OF_LAST_ACTION] = self.memory.get_recent_history(constants.KEY_REASON_OF_LAST_ACTION, k=1)[0]
            input[constants.SUCCESS_DETECTION] = self.memory.get_recent_history(constants.SUCCESS_DETECTION, k=1)[0]
