...
class Config(metaclass=Singleton):
    ...
    def __init__(self) -> None:
        ...
        # Parallel request to LLM parameters (serial rollback default)
        self.parallel_request_gather_information = False
    ...
    def _set_env_window_info(self):
        from cradle.gameio import gui_utils
        from cradle.gameio.lifecycle import ui_control
        ...
        env_window = namedtuple('A', ['left', 'top', 'width', 'height'])(
            left=0,
            top=0,
            width=self.DEFAULT_ENV_RESOLUTION[0],
            height=self.DEFAULT_ENV_RESOLUTION[1],
        )
        ...
        if len(named_windows) == 0:
            ...
            try:
                real_env_window = ui_control.select_window(named_windows)
            ...
            real_env_window = named_windows[0]

        if real_env_window is not None:
            gui_utils.check_window_conditions(real_env_window)
            env_window = real_env_window
    ...
    def _set_latest_memory_path(self):
        runs_dir = assemble_project_path('./runs')
        if not os.path.isdir(runs_dir):
            return

        path_list = [
            item for item in os.listdir(runs_dir)
            if os.path.isdir(os.path.join(runs_dir, item))
        ]
        path_list.sort()
        if len(path_list) != 0:
            latest_path = os.path.join(runs_dir, path_list[-1])
            self.skill_local_path = latest_path
            self.memory_load_path = latest_path
