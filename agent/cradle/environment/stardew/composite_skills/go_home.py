...
DEFAULT_GO_HOME_ITERATIONS = 5
DEFAULT_GO_HOME_TERMINAL_THRESHOLD = 90
...
@register_skill("go_home")
def go_home():
    """
    Move the character enter the house. This function automates the action of moving the character to enter the house by navigating through the door.
    Note: The action will take effect only if the house is visiable.
    """
    return go_to_icon("house_door", iterations=DEFAULT_GO_HOME_ITERATIONS, terminal_threshold=DEFAULT_GO_HOME_TERMINAL_THRESHOLD, debug=False, gm=None, character=["up", "down", "left", "right"])
...
def go_to_icon(target: str = "store", iterations=None, terminal_threshold=None, debug: bool = False, gm=None, character=None):
    ...
    if missing_assets:
        logger.warn(f"go_home missing template assets: {', '.join(missing_assets)}")
        return False
...
def cv_go_to_icon(
    ...
    for step in range(iterations):
        ...
        duration = 0.1
        logger.debug(f'go_home step: {step} dis:{(dis_x, dis_y)}')
        gap_threshold = 0
        if dis_x > 10:
            move_right(duration)
        elif dis_x < -10:
            move_left(duration)
        elif dis_y < -gap_threshold:
            move_up(duration)
        elif dis_y > gap_threshold:
            move_down(duration)
        time.sleep(1)
...
def take_screenshot():
    with mss.mss() as sct:
        ...
        image_name = f'screen_go_home_interact_{current_time}.jpg'
