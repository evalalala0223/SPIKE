...
def buy_item(item_name: str = "Parsnip Seeds",
    ...
    """
    return cv_buy_item(item_name=item_name, item_count=item_count)
...
def cv_buy_item(item_name: str = "Parsnip Seeds",
    ...
    try:
        ...
        if not os.path.exists(assemble_project_path(item_shop_icon_path)):
            logger.write(f"Shop item template not found for buy_item: {item_shop_icon_path}")
            return False
        ...
        if item_bounding_box:
            return iterate_buy_item(item_bounding_box, item_name=item_name, item_count=item_count, debug=debug)
        return False
