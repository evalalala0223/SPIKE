...
def nop():
    """
    No operation. This skill does nothing and is used as a fallback when no valid action can be determined.
    It simply waits for a short period without performing any game action.
    Use this when the agent needs to pause and observe the current state before deciding on the next action.
    """
    time.sleep(0.5)
    logger.write("Executing nop (no operation) - waiting for next decision cycle")
