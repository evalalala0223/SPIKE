...
class SkillRegistry():
    ...
    def __init__(self,
        ...
        # Track the skill library file path for later updates
        ...
        if self.skill_from_default and os.path.exists(os.path.join(config.skill_local_path, self.skill_library_filename)):
            self.skill_library_path = os.path.join(config.skill_local_path, self.skill_library_filename)
            self.skills = self.load_skills_from_file(self.skill_library_path)
        else:
        ...
        for skill_name in skill_local.keys():
            ...
            if not regenerate_flag:
                ...
                skills[skill_name] = Skill(skill_name,
                                           ...
                                           skill_code_base64)
            else: # skill_code has been modified or embedding is invalid, we should recompute embedding
                logger.write(f"Regenerate skill {skill_name}")
                # If skill is in skill_registered, use the original function and only regenerate embedding
                if skill_name in self.skill_registered.keys():
                    skills[skill_name] = Skill(skill_name,
                                               self.skill_registered[skill_name].skill_function,
                    ...
                    # Skill not in registry, try to register from code
                    self.register_skill_from_code(
                        skill_local[skill_name].skill_code,
                        trusted_source=True,
                    )
                    # Add regenerated skill to the skills dict
    ...
    def extract_function_info(self, input_string: str = "open_map()"):
        ...
        if match:
            ...
            # 如果有位置参数，尝试将它们映射到实际的参数名
            if len(positional_args) > 0:
                # 尝试获取技能函数的参数签名
                if function_name in self.skills:
                    try:
                        skill_function = self.skills[function_name].skill_function
                        params = list(inspect.signature(skill_function).parameters.keys())
                        ...
                        logger.warning(f"Could not map positional args for {function_name}: {e}")
                        # 如果映射失败，使用索引作为fallback
                        for i, arg in enumerate(positional_args):
                            if f'arg{i}' not in arguments and i not in [j for j in range(len(arguments))]:
                                arguments[f'arg{i}'] = arg
                else:
                    # 技能不在库中，使用索引作为key
                    for i, arg in enumerate(positional_args):
        ...
        if docstring:
            ...
            if len(params) > 0:
                ...
                for param in params.values():
                    name = param.name
                    match = re.search(rf"- {name}: (.+).", docstring)
                    param_description = match.group(1) if match else f"Parameter: {name}"
    ...
    def register_skill_from_code(
        ...
        Args:
            ...
            trusted_source: only trusted call sites may execute dynamic skill code.
        ...
        if not trusted_source:
            info = "Refusing to register skill code from an untrusted source."
            logger.warn(info)
            return False, info
        ...
        try:
            ...
            skill = namespace.get(skill_name)
            if skill is None:
                raise NameError(skill_name)
        except Exception:
    ...
    def retrieve_skills(self, query_task: str, skill_num: int, screen_type: str) -> List[str]:
        ...
        # 修复Bug：如果query_task为空或None，使用默认描述避免embedding API报错
        if not query_task or query_task.strip() == '':
            logger.warn("retrieve_skills received empty query_task, using default description")
            query_task = "general game task"
        
        task_emb = np.array(self.embedding_provider.embed_query(query_task))
        task_emb_dim = task_emb.shape[0]
        
        # Check for embedding dimension mismatch and regenerate if needed
        dimension_mismatch = False
        for skill_name, skill in self.skills.items():
            if skill.skill_embedding is not None and len(skill.skill_embedding) != task_emb_dim:
                dimension_mismatch = True
                logger.warn(f"Embedding dimension mismatch detected: skill has {len(skill.skill_embedding)}D, query has {task_emb_dim}D")
                break
        
        if dimension_mismatch:
            logger.write("Regenerating all skill embeddings to match current embedding model...")
            for skill_name, skill in self.skills.items():
                # Regenerate embedding using current model
                new_embedding = self.get_embedding(skill_name, inspect.getdoc(skill.skill_function))
                skill.skill_embedding = new_embedding
                logger.debug(f"Regenerated embedding for skill: {skill_name}")
            
            # Save updated skills to file to avoid regenerating next time
            if self.skill_library_path:
                logger.write(f"Saving updated skills with new embeddings to {self.skill_library_path}")
                self.store_skills_to_file(self.skill_library_path, self.skills)
