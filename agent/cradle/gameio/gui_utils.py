...
if _isWin():
    ...
    # Lazy initialization of AHK instance
    _ahk = None
...
class TargetWindow():
    ...
    def activate(self):
        ...
        if _isWin():
            try:
                ...
                if self._activate_win_fallback():
                    return
                raise e
        elif _isMac():
            env_name = self.title
    ...
    def _activate_win_fallback(self) -> bool:
        if not _isWin():
            return False
        ...
        try:
            return bool(self.window.isActive) or user32.GetForegroundWindow() == hwnd
        except Exception:
...
def check_window_conditions(env_window: TargetWindow):
    ...
    if env_window.width >= 300 and env_window.width <= 355 and env_window.height >= 130 and env_window.height <= 200:
        ...
        if not config._min_resolution_check(env_window):
            print(f"Warning: The resolution of env window {env_window.width}x{env_window.height} is lower than recommended 1920x1080.")
        if not config._aspect_ration_check(env_window):
            print(f"Warning: The screen ratio is not 16:9.")
