...
class AugmentProvider(BaseProvider):
    ...
    def __call__(self,
        ...
        # Debug: Log what augment is reading
        # logger.write(f"[DEBUG] augment: Reading IMAGES_MEM_BUCKET from working_area: {screenshot_path}")

        if screenshot_path is None or not os.path.exists(screenshot_path):
            logger.error(f"screenshot_path {screenshot_path} not exists or is None")
            augmented_screenshot_path = screenshot_path
        else:
            augmented_screenshot_path = screenshot_path.replace(".jpg", "_augmented.jpg")

            # Always read from original screenshot to prevent double-augmentation
        ...
        res_params = {
            ...
            "screenshot_augmented_path": augmented_screenshot_path
        }
