...
class VideoClipProvider(BaseProvider):
    ...
    def __init__(self, gm, video_recorder=None):

        super(VideoClipProvider, self).__init__()

        self.gm = gm
        self.memory = LocalMemory()
        self.video_recorder = video_recorder if video_recorder is not None else VideoRecordProvider()
    ...
    def __call__(self,
        ...
        if init:
            ...
            start_frame_id = self.memory.get_latest("start_frame_id", 0)
            end_frame_id = self.memory.get_latest("end_frame_id", start_frame_id)
            video_clip_path = self.video_recorder.get_video(start_frame_id, end_frame_id)
