...
class SkillCurationProvider(BaseProvider):
    ...
    def __call__(self,
        ...
        # 修复Bug：如果task_description仍为空，使用默认描述
        if not task_description or task_description.strip() == '':
            task_description = params.get('task', params.get('long_horizon_task', 'general gameplay task'))
            logger.debug(f"skill_curation: task_description was empty, using fallback: '{task_description}'")

        self.gm.register_generated_skills(all_generated_actions)

        # Ensure task_description is not empty before retrieval
        if not task_description or task_description.strip() == '':
            task_description = "general game task"
            logger.write(f'[WARNING] task_description is empty in generate_new, using default: {task_description}')
...
class RDR2SkillCurationProvider(BaseProvider):
    ...
    def __call__(self,
        ...
        if not task_description or task_description.strip() == '':
            task_description = "general game task"
            logger.write(f'[WARNING] task_description is empty, using default: {task_description}')
        ...
        return {"skill_library": skill_library}
