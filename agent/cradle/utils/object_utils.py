...
# Get the project root directory (Cradle/)
...
class GroundingDINO(metaclass=Singleton):
    ...
    def __init__(self):
        ...
        config_path = os.path.join(PROJECT_ROOT, "cache", "GroundingDINO_SwinB_cfg.py")
        weights_path = os.path.join(PROJECT_ROOT, "cache", "groundingdino_swinb_cogcoor.pth")
        self.detect_model = load_model(config_path, weights_path)
...
# Lazy initialization: only create instances when actually used
# This avoids loading models at import time
_groundingdino = None
_circle_detector = None

def get_groundingdino():
    """Lazy initialization of GroundingDINO instance"""
    global _groundingdino
    if _groundingdino is None:
        _groundingdino = GroundingDINO()
    return _groundingdino

def get_circle_detector():
    """Lazy initialization of CircleDetector instance"""
    global _circle_detector
    if _circle_detector is None:
        _circle_detector = CircleDetector()
    return _circle_detector

# Export the detect methods for backward compatibility
def groundingdino_detect(*args, **kwargs):
    return get_groundingdino().detect(*args, **kwargs)

def circle_detector_detect(*args, **kwargs):
    return get_circle_detector().detect(*args, **kwargs)