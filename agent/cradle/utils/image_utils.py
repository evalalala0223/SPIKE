...
# Removed: from cradle.gameio import IOEnvironment - causes circular import
from cradle.log import Logger
from cradle import constants

config = Config()
# Removed: io_env = IOEnvironment() - never used and causes circular import
logger = Logger()
...
def crop_grow_image(image_path: str, crop_area_border: tuple[int, int, int, int] = (8, 1, 8, 8), overwrite_flag: bool = False):
    ...
    with Image.open(image_path) as img:
        ...
        cropped_img = cropped_img.resize(config.DEFAULT_ENV_RESOLUTION, Image.LANCZOS)
...
def draw_mask_panel(image: Image, **kwargs):
    ...
    # Check if image is large enough for mask panel processing
    # Required minimum size: 475x315 (based on bottom_right_green)
    if height < 315 or width < 475:
        logger.write(f"[WARNING] Image too small for mask panel ({width}x{height}), skipping. Minimum required: 475x315")
        return image
