            ...
            if self.frame_extractor is not None:
                ...
                try:
                    ...
                    if isinstance(target_frame_count, int) and target_frame_count > 0:
                        ...
                        logger.write(f"[Performance] Using {len(extracted_frame_paths)} frames (target: {target_frame_count})")
                except Exception as e:
                    logger.warn(f"[Performance] Failed to apply target_frame_count: {e}")
