...
from stardojo.utils.encoding_utils import decode_base64
from stardojo.utils.string_utils import hash_text_sha256
from stardojo.config import Config
from stardojo.utils.file_utils import get_latest_directories_in_path
    ...
    for i in range(len(encoded_images)):
        if len(image_paths) == 1 and len(encoded_images) > len(image_paths):
            if isinstance(image_paths, dict):
                replacement_path = next(iter(image_paths.values()))
            else:
                replacement_path = image_paths[0]
            text = text.replace(encoded_images[i], replacement_path)
        else:
            ...
            if not isinstance(image_paths, dict):
                image_paths = {str(idx): path for idx, path in enumerate(image_paths)}
