...
from stardojo.utils.file_utils import assemble_project_path, get_project_root
...
class Config(metaclass=Singleton):
    ...
    skill_configs = {
        ...
        constants.SKILL_CONFIG_ALLOW_GENERATED_REGISTRATION: False,
    ...
    VideoFrameExtractor_path = "./agent/res/tool/subfinder/VideoSubFinderWXW.exe"
    VideoFrameExtractor_placeholderfile_path = "./agent/res/tool/subfinder/test.srt"
    ...
    def __init__(self) -> None:
        ...
        self.max_turn_count = 20
        self.checkpoint_interval = 1

        self.temperature = self.DEFAULT_TEMPERATURE
        self.seed = self.DEFAULT_SEED
        self.fixed_seed = os.getenv("FIXED_SEED", "false").lower() in ("true", "1", "yes")
        ...
        self.action_planning_image_num = int(os.getenv("ACTION_PLANNING_IMAGE_NUM", "2"))
        self.number_of_execute_skills = max(1, int(os.getenv("NUMBER_OF_EXECUTE_SKILLS", "2")))
        self.skill_library_with_code = False

        # OCR local checks
        self.ocr_fully_ban = True # whether to fully turn-off OCR checks
        self.ocr_enabled = False # whether to enable OCR during long-running action loops
        ...
        self.composite_terminal_skill_names = []
        self.composite_skill_wait_seconds = {}

        # Change max steps, Self-reflection image count for software
        # self.max_turn_count = 20 if self.is_game == False else 999999
        ...
        self.env_resolution = self.DEFAULT_ENV_RESOLUTION
        self.env_region = (0, 0, self.DEFAULT_ENV_RESOLUTION[0], self.DEFAULT_ENV_RESOLUTION[1])
        self.base_resolution = (3840, 2160)
        self.resolution_ratio = self.DEFAULT_ENV_RESOLUTION[0] / self.base_resolution[0]
        self.mouse_move_factor = 1.0
        self.screen_resolution = self.DEFAULT_ENV_RESOLUTION


    def load_env_config(self, env_config_path):
        """Load environment specific configuration."""
        from stardojo.gameio import gui_utils

        raw_path = str(env_config_path or "").strip()
        candidate_paths = []
        if raw_path:
            if os.path.isabs(raw_path):
                ...
                candidate_paths.append(assemble_project_path(raw_path))

                normalized = raw_path.replace("\\", "/")
                if normalized.startswith("agent/"):
                    candidate_paths.append(
                        assemble_project_path(normalized.split("/", 1)[1])
                    )

        path = next((candidate for candidate in candidate_paths if os.path.exists(candidate)), None)
        if not path:
            path = assemble_project_path(raw_path)
        ...
        self.env_sub_path = kget(self.env_config, constants.ENVIRONMENT_SUB_PATH, default='')
        self.env_short_name = kget(self.env_config, constants.ENVIRONMENT_SHORT_NAME, default='')
        ...
        # Skill retrieval
        self.skill_local_path = './agent/res/' + self.env_sub_path + '/skills/'
    ...
    def set_work_dir_base(self, log_dir_name=None):
        self.work_dir_base = './runs'
        if log_dir_name is not None:
            self.work_dir_base = os.path.join(self.work_dir_base, log_dir_name)

    def _set_dirs(self) -> None:
        """Setup directories needed for one system run."""
        self.root_dir = get_project_root()

        self.work_dir = assemble_project_path(self.work_dir_base)

    def set_logger_dirs(self) -> None:
        """Setup directories needed for one system run."""
    ...
    def _set_env_window_info(self):
        ...
        from stardojo.gameio import gui_utils
        from stardojo.gameio.lifecycle import ui_control
        from stardojo.gameio.gui_utils import TargetWindow

        # Fake target environment window info for testing cases with no running env application
        EnvWindow = namedtuple('A', ['left', 'top', 'width', 'height'])
        env_window = EnvWindow(
            0,
            0,
            self.DEFAULT_ENV_RESOLUTION[0],
            self.DEFAULT_ENV_RESOLUTION[1]
        )
        ...
        if len(named_windows) == 0:
            ...
            try:
                env_window = ui_control.select_window(named_windows)
        ...
        if hasattr(env_window, "window"):
            gui_utils.check_window_conditions(cast(TargetWindow, env_window))
