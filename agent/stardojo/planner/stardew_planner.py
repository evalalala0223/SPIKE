...
def _llm_stage_kwargs(llm_provider: Any, stage: str, **kwargs) -> Dict[str, Any]:
    call_kwargs = dict(kwargs)
    if callable(getattr(llm_provider, "_resolve_request_controls", None)):
...
async def gather_information_get_completion_parallel(llm_provider, text_input_map, current_frame_path, time_stamp,
    ...
    while not success_flag and retry_count < max_retries:
        try:
            retry_temperature = min(1.0, config.temperature + 0.3 * retry_count)
            response, info = await llm_provider.create_completion_async(
                message_prompts,
                **_llm_stage_kwargs(
                    llm_provider,
                    "llm_description",
            ...
            retry_count += 1
            logger.warn(f"[Planner] Retry {retry_count}: parse failed, response was: {response[:200] if response else 'None'}")
            logger.error(f"Response is not in the correct format: {e}, retrying...")
            success_flag = False
            if retry_count >= max_retries:
                logger.error(f"Max retries reached in gather_information_get_completion_parallel: {max_retries}")
                return False
    ...
    # Convert the response to dict
    if processed_response is None or not response:
...
def gather_information_get_completion_sequence(llm_provider, text_input_map, current_frame_path, time_stamp,
    ...
    while not success_flag and retry_count < max_retries:
        try:
            retry_temperature = min(1.0, config.temperature + 0.3 * retry_count)
            response, info = llm_provider.create_completion(
                message_prompts,
                **_llm_stage_kwargs(
                    llm_provider,
                    "llm_description",
            ...
            success_flag = False
            if retry_count >= max_retries:
                logger.error(f"Max retries reached in gather_information_get_completion_sequence: {max_retries}")
                return False
...
class InformationGathering():
    ...
    def __init__(
            ...
            text_input_map: Optional[Dict[str, Any]] = None,
            get_text_template: Optional[str] = None,
            toolbar_input_map: Optional[Dict[str, Any]] = None,
    ...
    def __call__(self, *args, input: Optional[Dict[str, Any]] = None, class_=None, **kwargs) -> Dict[str, Any]:
        input = cast(Dict[str, Any], self.input_map if input is None else input)
        ...
        if gather_information_configurations[constants.FRAME_EXTRACTOR] is True:
            ...
            all_task_guidance: List[Dict[str, Any]] = []
            if frame_extractor_gathered_information is not None:
        ...
        processed_response = processed_response or {}
        ...
        if gather_information_configurations[constants.OBJECT_DETECTOR] is True:
            ...
            if self.object_detector is not None:
                try:
                    target_object_value = str(processed_response.get(constants.TARGET_OBJECT_NAME, ""))
                    target_object_name = target_object_value.lower() \
                        if constants.NONE_TARGET_OBJECT_OUTPUT not in target_object_value.lower() else ""
    ...
    async def template_matching_for_current_toolbar(self, sr_file_list, base_template_file_list, work_template_file_list) -> Tuple[Dict[str, str], Optional[int]]:
        ...
        for sr_file in sr_file_list:
            ...
            if is_selected:
                ...
                break
        for key, matched_path in list(matching_dict.items()):
            if isinstance(matched_path, str):
                matching_dict[key] = os.path.splitext(os.path.basename(matched_path))[0]
        return matching_dict, selected_position

    async def gather_toolbar_list(self, match_dict: Dict[str, str], get_number_flag: bool = True) -> List[Dict[str, Any]]:
        if not match_dict:
            logger.warn("[InfoGathering] Toolbar matching returned empty result")
            return []
    ...
    def extract_number(self, data) -> Optional[int]:
        for item in data:
            if None in item:
    ...
    async def gather_llm_description(self, input):
        ...
        if gather_information_configurations[constants.LLM_DESCRIPTION] is True:
            ...
            try:
                ...
                response = None  # 防止 API 异常时 response 未定义 (UnboundLocalError fix)
                while gather_information_success_flag is False and gather_info_retry_count < gather_info_max_retries:
                    try:
                        retry_temperature = min(1.0, config.temperature + 0.3 * gather_info_retry_count)
                        if hasattr(self.llm_provider, 'create_completion_async'):
                            response, info = await self.llm_provider.create_completion_async(
                                message_prompts,
                                **_llm_stage_kwargs(
                                    self.llm_provider,
                                    "llm_description",
                            ...
                            response, info = await asyncio.to_thread(
                                self.llm_provider.create_completion,
                                message_prompts,
                                **_llm_stage_kwargs(
                                    self.llm_provider,
                                    "llm_description",
                        ...
                        logger.warn(f"[Planner] Retry {gather_info_retry_count}: parse failed, response was: {response[:200] if response else 'None'}")
...
class LLMProcessingStep():
    """Base class for LLM processing steps (action planning, success detection, etc.).

    Subclasses only need to set ``step_name`` to get correct log messages.
    Override ``_pre`` / ``_post`` for step-specific processing.
    """

    step_name: str = "llm_processing_step"
    ...
    def __call__(self, *args, input: Optional[Dict[str, Any]] = None, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            response, info = self.llm_provider.create_completion(
                message_prompts,
                **_llm_stage_kwargs(self.llm_provider, self.step_name),
            )

            logger.debug(f'{logger.DOWNSTREAM_MASK}{response}\n')

            if response is None or len(response) == 0:
                logger.warn(f'No response in {self.step_name} call')
                logger.debug(input)
            ...
            logger.error(f"Error in {self.step_name}: {e}")
            logger.error_ex(e)
            flag = False
...
class ActionPlanning(LLMProcessingStep):
    step_name = "action_planning"


class SuccessDetection(LLMProcessingStep):
    step_name = "success_detection"


class SelfReflection(LLMProcessingStep):
    step_name = "self_reflection"


class TaskInference(LLMProcessingStep):
    step_name = "task_inference"
...
class StardewPlanner(BasePlanner):
    ...
    def set_internal_params(self,
        ...
        if planner_params is None or not check_planner_params(planner_params):
            raise ValueError(f"Error in planner_params: {planner_params}")
    ...
    def self_reflection(self, *args, input: Optional[Dict[str, Any]] = None, **kwargs) -> Dict[str, Any]:
        ...
        if self.self_reflection_ is None:
            return {"flag": False, "input": input, "res_dict": {}}
