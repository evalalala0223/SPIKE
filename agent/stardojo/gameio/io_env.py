...
from stardojo.gameio.gui_utils import get_named_windows, get_named_windows_fallback, get_screen_size, mouse_button_down, mouse_button_up, key_down, key_up, mouse_wheel_scroll, type_keys, mouse_click, get_mouse_location, mouse_move_to, get_active_window
...
class IOEnvironment(metaclass=Singleton):
    ...
    def _resolve_env_window(self):
        ...
        if named_windows:
            if len(named_windows) == 1:
                ...
                try:
                    from stardojo.gameio.lifecycle.ui_control import select_window
    ...
    def _verify_game_focused_or_abort(self):
        """Verify the game window is currently focused RIGHT NOW.

        Returns True if the game window holds focus and it is safe to send
        input. Returns False if we should skip the keystroke to avoid leaking
        it into the user's active window.

        This is a tight check done immediately before key_down/key_up to
        close the race between window activation and keystroke delivery.
        """
        if not getattr(config, "is_game", False):
            # Non-game contexts don't use the shared desktop, let them pass.
            return True

        env_window = self._resolve_env_window()
        if env_window is None:
            logger.warn("Refusing keystroke: game window not resolvable.")
            return False
        ...
        for attempt in range(self.FOCUS_VERIFY_RETRIES + 1):
            ...
            # Not focused. Try to reactivate and wait, but only up to retries.
    ...
    def key_press(self, key, duration=None):
        ...
        if f == True:
            ...
            # Tight focus check right before the keystroke to close the race
            # between window activation and global pydirectinput delivery.
            if not self._verify_game_focused_or_abort():
                return False
    ...
    def key_hold(self, key, duration=None):
        ...
        if f == True:
            ...
            # Tight focus check before sending global keystroke.
            if not self._verify_game_focused_or_abort():
                return False
