...
from stardojo.utils.string_utils import strip_anchor_chars
...
if _isWin():
    ...
    def _resolve_ahk_executable_path() -> str:
        env_path = str(os.environ.get("AHK_PATH", "") or "").strip()
    ...
    def get_ahk():
        """Lazily create the AutoHotkey bridge only when a caller needs it."""
        global _ahk
        if _ahk is None:
            executable_path = _resolve_ahk_executable_path()
            try:
                _ahk = AHK(executable_path=executable_path) if executable_path else AHK()
            except Exception as exc:
                raise RuntimeError(
                    "AutoHotkey is required for Windows mouse button automation. "
                    "Install AutoHotkey or set AHK_PATH to AutoHotkey.exe."
                ) from exc
        return _ahk
...
def mouse_button_down(button):
    if _isWin() and config.is_game is True:
        get_ahk().click(button=button, direction='D')
    else:
...
def mouse_button_up(button):
    if _isWin() and config.is_game is True:
        get_ahk().click(button=button, direction='U')
    else:
...
def mouse_click(click_count, button, relative=False):
    if _isWin() and config.is_game is True:
        get_ahk().click(click_count=click_count, button=button, relative=relative)
    else:
...
def get_mouse_location(absolute = False):
    if _isWin() and config.is_game is True:
        return get_ahk().get_mouse_position()
    else:
