...
from stardojo.utils.string_utils import contains_punctuation, is_numbered_bullet_list_item
...
def serialize_data(item):
    ...
    # Lazy torch import to avoid 2-5s startup penalty
    try:
        import torch as _torch
    except ImportError:
        _torch = None

    if _torch is not None and isinstance(item, _torch.Tensor):
        if item.dim() == 0:
            return item.item()
        else:
            if item.is_cuda:
    ...
    elif isinstance(item, JsonFrameStructure):
        return item.to_dict()
    return item
