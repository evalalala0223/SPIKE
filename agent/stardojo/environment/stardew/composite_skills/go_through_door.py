...
import time
import random
from stardojo.environment.stardew.atomic_skills.basic_skills import (
    use_tool,
    do_action,
    move_up,
    move_down,
...
from stardojo.environment.stardew.composite_skills.go_home import go_home
config = Config()
logger = Logger()


DEFAULT_GO_THROUGH_DOOR_ITERATIONS = 30
DEFAULT_GO_THROUGH_DOOR_TERMINAL_THRESHOLD = 95


@register_skill("go_through_door")
def go_through_door(door):
    """
    Move the character to go through a door. This function automates the action of moving the character to enter/leave a building by navigating through the door. You can use go_through_door(door="store_entrance") to enter the store, use go_through_door(door="store_exit") to leave the store, use go_through_door(door="home_entrance") to enter your house and use go_through_door(door="home_exit") to leave your house.
    Note: This function will take effect only if the door is visiable.

    Parameters:
    - door: The door which the character want to go through. There are only four choices: home_entrance, home_exit, store_entrance and store_exit. For example, if the character wants to enter the store, then door should be store_entrance.
    """
    return go_to_icon(
        door,
        iterations=DEFAULT_GO_THROUGH_DOOR_ITERATIONS,
        terminal_threshold=DEFAULT_GO_THROUGH_DOOR_TERMINAL_THRESHOLD,
        debug=False,
        gm=None,
...
def go_to_icon(target: str = "store", iterations=None, terminal_threshold=None, debug: bool = False, gm=None, character=None):
    ...
    """
    template_file = f'./res/{config.env_sub_path}/icons/{target}.jpg'
    character_files = [f'./res/{config.env_sub_path}/icons/{ch}.jpg' for ch in character]
    required_assets = [template_file, *character_files]
    missing_assets = [asset for asset in required_assets if not os.path.exists(assemble_project_path(asset))]
    if missing_assets:
        logger.warn(f"go_through_door missing template assets: {', '.join(missing_assets)}")
        return False
    return cv_go_to_icon(
        iterations,
...
def match_template(src_file, template_file, template_resize_scale=1, debug=False, character=None):
    ...
    if character == None:
        ...
        #     logger.write(f"Can not find character, max_confidence={max_confidence}")
        #     return None, None
    ...
    detection = matchTemplates([('', cv2.resize(template, (0, 0), fx=s, fy=s)) for s in [1]],
                               ...
                               method=cv2.TM_CCOEFF_NORMED,
                               maxOverlap=0.1)
    (x, y, w, h), confidence = detection['BBox'].iloc[0], detection['Score'].iloc[0]
...
def cv_go_to_icon(
    ...
    for step in range(iterations):
        ...
        if dis_xy is None:
            logger.write("Door template not matched at close range. Trying direct interaction before re-observing.")
            do_action()
            if "home_entrance" in template_file:
                go_home()
            elif lastmove != None:
                lastmove(duration=0.3)
            logger.write("Door transition not visually confirmed; forcing re-observation instead of reporting success.")
            return False
        ...
        if dis < terminal_threshold:  # begin to settle
            ...
            if "home_entrance" in template_file:
                ...
                lastmove(duration=0.3)
            follow_up_screenshot_path = take_screenshot()
            follow_up_dis_xy, _ = match_template(
                follow_up_screenshot_path,
                template_file,
                debug=debug,
                character=character,
            )
            if follow_up_dis_xy is None:
                logger.write("Door transition visually confirmed after interaction.")
                return True
            logger.write("Door template still visible after interaction; transition not confirmed.")
            return False
        ...
        # duration = random.random() * 0.2 + 0.2
        duration = 1
        logger.debug(f'go_through_door step: {step} dis:{(dis_x, dis_y)}')
        gap_threshold = 10
        speed = 410
        if dis_x > gap_threshold:
            move_right(duration * abs(dis_x) / speed)
            lastmove = move_right
        elif dis_x < -gap_threshold:
            move_left(duration * abs(dis_x) / speed)
            lastmove = move_left
        if dis_y < 0:
            move_up(duration * abs(dis_y) / speed + 0.3)
            lastmove = move_up
        elif dis_y > 0:
            move_down(duration * abs(dis_y) / speed + 0.3)
            lastmove = move_down
        time.sleep(0.5)
    ...
    logger.write(f'Go to icon failed to reach icon.')
    return False  # failed
...
def take_screenshot():
    with mss.mss() as sct:
        ...
        image_name = f'screen_go_through_door_interact_{current_time}.jpg'
...
if __name__ == "__main__":
    go_through_door(door="store_exit")
