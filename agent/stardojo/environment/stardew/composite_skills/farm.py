...
def enter_door_and_sleep():
    """
    Let the character enter the house and then walk onto the bed and interact to go to sleep. The bed is a walkable tile — the character moves onto it, then uses interact to trigger sleep.
    """
    move_up(duration=1)
    do_action()
    time.sleep(2)
...
def use_tool_multiple_times():
    ...
    """
    return move_and_action_like(action=use_tool, size=(3, 3))
...
def do_action_multiple_times():
    ...
    """
    return move_and_action_like(action=do_action, size=(3, 3))
