...
# from stardojo.environment.rdr2.atomic_skills.move import turn, move_forward
# from stardojo.environment.rdr2.lifecycle.ui_control import take_screenshot
...
from stardojo.utils.template_matching import match_template_image

config = Config()
logger = Logger()


DEFAULT_GO_TO_STORE_ITERATIONS = 15
...
def go_to_store():
    ...
    return go_to_icon(
        "store_door",
        iterations=DEFAULT_GO_TO_STORE_ITERATIONS,
        terminal_threshold=DEFAULT_GO_TO_STORE_TERMINAL_THRESHOLD,
        debug=False,
        gm=None,
...
def go_to_icon(target: str = "store", iterations=None, terminal_threshold=None, debug: bool = False, gm=None, character=None):
    ...
    if missing_assets:
        logger.warn(f"go_to_store missing template assets: {', '.join(missing_assets)}")
        return False
...
def cv_go_to_icon(
    ...
    for step in range(iterations):
        ...
        # 3. Move toward the icon
        duration = 0.1
        logger.debug(f'go_to_store step: {step} dis:{(dis_x, dis_y)}')
        gap_threshold = 20
