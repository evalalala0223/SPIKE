...
from stardojo.environment.utils import serialize_skills, deserialize_skills
from stardojo.utils.check import is_valid_value
from stardojo.gameio.io_env import IOEnvironment
from stardojo.constants import *
...
class SkillRegistry():
    ...
    @staticmethod
    def _safe_eval_argument(node):
        try:
            return ast.literal_eval(node)
        except ValueError:
            if isinstance(node, ast.Name):
                normalized = str(node.id).strip().lower()
    ...
    def load_skills_from_scripts(self) -> Dict[str, Skill]:
        ...
        # Try to load cached embeddings to avoid recomputing on every startup
        cache_path = os.path.join(config.skill_local_path, self.skill_library_filename)
        cached_skills = {}
        if os.path.exists(cache_path):
            try:
                cached_raw = load_json(cache_path)
                cached_skills = deserialize_skills(cached_raw)
                logger.debug(f"Loaded {len(cached_skills)} cached skill embeddings from {cache_path}")
            except Exception:
                cached_skills = {}
        ...
        for skill_name in self.skill_registered.keys():
            ...
            if not is_valid_value(skill_embedding): # The skill_embedding is invalid
                # Check if we have a valid cached embedding for unchanged code
                if skill_name in cached_skills:
                    cached = cached_skills[skill_name]
                    cached_base64 = base64.b64encode(cached.skill_code.encode('utf-8')).decode('utf-8')
                    if cached_base64 == skill_code_base64 and is_valid_value(cached.skill_embedding):
                        skill_embedding = cached.skill_embedding
                        regenerate_flag = False

                if not is_valid_value(skill_embedding):
                    regenerate_flag = True
    ...
    def extract_function_info(self, input_string: str = "open_map()"):
        ...
        Output format:
        {
            "x": ...,
            "y": ...,
            "other_params": [..., ...] //Unlabeled paramaters 
        }
        '''
        ...
        if match:
            ...
            positional = []
            # triverse ast
            call_node = parsed_arguments.body
            if not isinstance(call_node, ast.Call):
                raise ValueError("Parsed arguments do not form a function call.")
            
            for arg in call_node.args:
                try:
                    positional.append(self._safe_eval_argument(arg))
                except ValueError:
                    raise ValueError(f"Unable to evaluate positional argument: {ast.dump(arg)}")

            
            for keyword in call_node.keywords:
                try:
                    arguments[keyword.arg] = self._safe_eval_argument(keyword.value)
                except ValueError:
                    raise ValueError(f"Unable to evaluate keyword argument '{keyword.arg}': {ast.dump(keyword.value)}")
                    
            arguments["other_params"] = positional

            if len(raw_arguments) > 0 and len(arguments.keys()) == 0 and not positional:
    ...
    def execute_skill(self,
        ...
        try:
            positional = skill_params["other_params"]
            skill_params.pop("other_params")
            if executer is not None and hasattr(executer, skill_name):
                skill_response = getattr(executer, skill_name)(*positional, **skill_params)
            elif skill_name in self.skills:
                skill_function = self.skills[skill_name].skill_function
                skill_response = skill_function(*positional, **skill_params)
            else:
